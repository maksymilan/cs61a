...
def pingpong(n):
    ...
    True
    """
    def pingpong_helper(k,step,result):#k代表下标，step代表步长，result代表下标的对应值
        if n==1:
            return 1
        elif k==n:
            return result
            ...
            return pingpong_helper(k+1,step,result+step)
    return pingpong_helper(1,1,1)#从1到n进行递归
...
def missing_digits(n):
    ...
    def miss_count(m):
        if m<10:
            ...
            if m%10!=m//10%10:
                return m%10-m//10%10-1+miss_count(m//10)
            else:
                return miss_count(m//10)
    return miss_count(n)
